...
{
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4|GPIO_Pin_5|GPIO_Pin_6|GPIO_Pin_8|GPIO_Pin_10;
...
/*******************************************************************************
  ...
  ********************************************************************************/
u8 TCS230_CurrentColor(u8 pRGB[3])
{
	u8 currentcolor=0;
...
/*******************************************************************************
 ...
 unsigned int Robot_checkColor(void)             
 {
	  ...
	  //printf("Red: %d  Blue: %d  Green: %d\n",RGB[0],RGB[1],RGB[2]);
		...
		else if(((abs(RGB[0] - RGB[2])<50)&&(abs(RGB[0] - RGB[1])<50)&& (RGB[0]>100) ) || ( RGB[0]-RGB[2]<15 && RGB[1]>200 ))
		{
			return 2;//白色
		}
		else if((abs(RGB[0] - RGB[2])<30)&&(abs(RGB[0] - RGB[1])<30)&& (RGB[0]<40)&& (RGB[1]<40) )
		{
			return 4;//黑色
		}
		...
		do
		{
			...
			distance = GetDis(GPIO_Pin_12,GPIO_Pin_13);
				if(distance > 4 && QTIS == 15){
					have = false;
					Gback();
					process=2;     //*****待改******待改***********待改******待改***********待改*********************待改************************8888
					return;
				}
...
if(!have){
		...
		pulses = 7;                      //前冲步数   可修改  优化
		while(pulses--)
		{
			motor_motion2(1550, 1450);	  //直行
		}
		TurnLeft_nDegree(103);//左转n度
		do
		{
			motor_motion2(1550, 1450);	  //直行
			QTIS = (GPIO_ReadInputData(GPIOE)&0x0f);
...
{		
	...
	printf("hadedone:%d",hadedone);
	  ...
	  do
		{
			Robot_hunting1(1550);
		}while(QTIS);                             //当第一次检测到白点跳出循环
		pulses = goal+1 ;             // 35 38 40 
		while(pulses--)
		{
			Robot_hunting1(1550);	  //直行
		}	
...
void CarryFpoint(void)
{
	if(Fhavesome==1)
	{
		...
		}while(distance > 4);  //机器朝着F点位置物块方向直走，直到距离小于3
		pulses=25;
		while(pulses--)
		{
			motor_motion2(1550, 1450);	  //直行
...
{		
						...
						process =6;	 //将C/B/A/D/E/G/H点有的色块搬到开始线上，如果比赛，只需要修改该参数即可  程序首先从case11开始
						
	          TCS230_WhiteBalance(pcolor);  //白平衡
	
					/*	while(true){
						
							
							TurnRight_nDegree(30);
							delay_nms(2000);
							TurnRight_nDegree(60);
							delay_nms(2000);
							TurnRight_nDegree(90);
							delay_nms(2000);
							TurnRight_nDegree(120);
   ...
   //     motor_motion2(1500, 1500); 		
			...
			{/**************一般1~10选着用不到********************************/
				case 1:	CCheck();
				        CarryCpoint();break;//超声波检测c点
				case 2:	BCheck();
				        CarryBpoint();break;//超声波检测b点
				case 3:	ACheck();
				        CarryApoint();break;//超声波检测a点
				case 4:	ICheck();
					      CarryIpoint();break;//超声波检测i点    //i点为颜色识别跳出程序所在之处
				case 5:	HCheck();
				        CarryHpoint();break;//超声波检测h点
				case 6: GCheck();
				        CarryGpoint();break;//超声波检测g点
				case 7:	FCheck();
				        CarryFpoint();break;//超声波检测f点
				case 8:	ECheck();
				        CarryEpoint();break;//超声波检测e点
				case 9: DCheck();
				        CarryDpoint();break;//超声波检测d点		
				//case 10:turn_back();break;
				case 11:StartCarryToScore();break;//将C/B/A/D/E/G/H点有的色块搬计分区
				case 12:moveBack();break;
				default:break;
			}
	}
}
