...
//			mod_f[0] = 1;//////////////////
		...
		else if (rc_in.rc_ch.st_data.ch_[ch_5_aux1] < 1400)
		{
			LX_Change_Mode(2);
//			mod_f[0] = 2;/////////////////
			my_code_control_flag=0;
...
//			LX_Change_Mode(3);
//			mod_f[0] = 3;
			LX_Change_Mode(2);/////////////////////////////定点飞行模式下才能使用实时控制帧///////////////////////////////////////////////////////
//			mod_f[0] = 2;/////////////////
			my_code_control_flag=1;//是否使用代码控制的标志位
		}
		/*
//注释，原因暂无	
//		//有切换模式时执行
...
//		}
		...
		if(my_code_control_flag==1)////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		{
			for(int i=0;i<4;i++)
			{
				if((rc_in.rc_ch.st_data.ch_[i]>1300)&&(rc_in.rc_ch.st_data.ch_[i]<1700))rc_in.rc_ch.st_data.ch_[i]=1500;//当摇杆都在中间才能使用实时控制帧
			}
		}
...
//		}
		//############(实时控制帧，自主开发闭环控制，在这里赋值即可)##############使用实时控制帧控制
		//实时XYZ-YAW期望速度(实时控制帧)
		if(my_code_control_flag==1)//当左摇杆下，右摇杆上，才进行赋值；当左摇杆下，右摇杆下时，不再赋值才能进行一键降落
		{
			if(rc_in.rc_ch.st_data.ch_[ch_6_aux2]>800 && rc_in.rc_ch.st_data.ch_[ch_6_aux2]<1200)
			{
				...
				*/
				...
				rt_tar.st_data.vel_x = (int)my_give_vel_x;    //头向速度，厘米每秒     (int)my_give_vel_y
				rt_tar.st_data.vel_y = -(int)my_give_vel_y;    //右向速度，厘米每秒
//				rt_tar.st_data.vel_y = 0;//不给y轴速度，只沿x轴方向进行走
...
static void Bat_Voltage_Data_Handle()
{
  ...
  fc_bat.st_data.voltage_100=1200;
//	fc_bat.st_data.voltage_100=(int)(1.526*(double)fc_bat.st_data.voltage_100);
}
...
void ANO_LX_Task()
{
	...
	if (tmp_cnt[0] == 0)
	{
		...
		//遥控数据处理
		RC_Data_Task(0.01f);//实时控制帧在里面写着，大概就是10ms进行扫描一次进行一次程控调节
		//飞控状态处理
